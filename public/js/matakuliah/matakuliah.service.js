class mataKuliahService {
    ...
    async getAllData() {
        ...
        try {
            const prodiResponse = await this.ajaxRequest(`${appUrl}/v1/prodi/`, `GET`);
            const prodiData = prodiResponse.data;
            console.log(prodiData, "Response data :")
            this.populateProdiDropdown(prodiData);

            const responseData = await this.ajaxRequest(`${appUrl}/v1/mata-kuliah/`, 'GET');
                ...
                responseData.data.forEach((item, index) => {
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <button class="btn btn-outline-primary btn-sm edit-modal mr-1" data-toggle="modal" data-target="#mataKuliahModal" data-id="${item.id}">
    ...
    populateProdiDropdown(prodiData){
        const ProdiSelect = $('#id_prodi');
        ProdiSelect.empty();
        ProdiSelect.append('<option value="" selected disabled hidden >- pilih -</option>');

        $.each(prodiData, function(index, prodi) {
            ProdiSelect.append(`<option value="${prodi.id}">${prodi.nm_prodi}</option>`);
        });
    }
    ...
    async upsertData(e, checkingEdit) {
        ...
        try {
            ...
            if (isEditMode) {
                const id = $('#id').val();
                const responseData = await this.ajaxRequest(`${appUrl}/v1/mata-kuliah/update/${id}`, 'POST', formData);
                ...
                const responseData = await this.ajaxRequest(`${appUrl}/v1/mata-kuliah/create`, 'POST', formData);
                console.log(responseData, 'Response create : ');
                if (responseData.status === 'success') {
    ...
    async getDataById(id, checkingEdit) {
        try {
            const responseData = await this.ajaxRequest(`${appUrl}/v1/mata-kuliah/get/${id}`, 'GET');
            console.log(checkingEdit())
            $('#modal-title').html('Edit Data')
            $('#id').val(responseData.data.id);
            $('#id_prodi').val(responseData.data.id_prodi);
            $('#mk').val(responseData.data.mk);
            $('#sks').val(responseData.data.sks);
            checkingEdit();
        } catch (error) {
            console.log(error);
        }
    ...
    async deleteData(id) {
        try {
            ...
            if (result.isConfirmed) {
                const responseData = await this.ajaxRequest(`${appUrl}/v1/mata-kuliah/delete/${id}`, 'DELETE');
                console.log(responseData);
                if (responseData.status === 'success') {
