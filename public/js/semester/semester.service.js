class semesterService {
    ...
    async getAllData() {
        ...
        try {
            const responseData = await this.ajaxRequest(`${appUrl}/v1/semester/`, 'GET');
                ...
                responseData.data.forEach((item, index) => {
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <button class="btn btn-outline-primary btn-sm edit-modal mr-1" data-toggle="modal" data-target="#semesterModal" data-id="${item.id}">
    ...
    async upsertData(e, checkingEdit) {
        ...
        try {
            ...
            if (isEditMode) {
                const id = $('#id').val();
                const responseData = await this.ajaxRequest(`${appUrl}/v1/semester/update/${id}`, 'POST', formData);
                ...
                const responseData = await this.ajaxRequest(`${appUrl}/v1/semester/create`, 'POST', formData);
                console.log(responseData);
                if (responseData.status === 'success') {
    ...
    async getDataById(id, checkingEdit) {
        try {
            const responseData = await this.ajaxRequest(`${appUrl}/v1/semester/get/${id}`, 'GET');
            console.log(checkingEdit())
            $('#modal-title').html('Edit Data')
            $('#id').val(responseData.data.id);
            $('#semester').val(responseData.data.semester);
            $('#tgl_mulai').val(responseData.data.tgl_mulai);
            $('#tgl_selesai').val(responseData.data.tgl_selesai);
            checkingEdit();
        } catch (error) {
            console.log(error);
        }
    ...
    async deleteData(id) {
        try {
            ...
            if (result.isConfirmed) {
                const responseData = await this.ajaxRequest(`${appUrl}/v1/semester/delete/${id}`, 'DELETE');
                console.log(responseData);
                if (responseData.status === 'success') {
