class kelasService {
    ...
    async getAllData() {
        ...
        try {
            const responseData = await this.ajaxRequest(`${appUrl}/v1/kelas/`, 'GET');
                ...
                responseData.data.forEach((item, index) => {
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <button class="btn btn-outline-primary btn-sm edit-modal mr-1" data-toggle="modal" data-target="#kelasModal" data-id="${item.id}">
    ...
    async upsertData(e, checkingEdit) {
        ...
        try {
            ...
            if (isEditMode) {
                const id = $('#id').val();
                const responseData = await this.ajaxRequest(`${appUrl}/v1/kelas/update/${id}`, 'POST', formData);
                ...
                const responseData = await this.ajaxRequest(`${appUrl}/v1/kelas/create`, 'POST', formData);
                console.log(responseData);
                if (responseData.status === 'success') {
                ...
                } else if (responseData.message === 'Nama kelas sudah ada'){
                    existAlert()
                } else {
                    errorAlert()
                }
            ...
            if (error.responseJSON.message === 'Nama kelas sudah ada'){
                existAlert()
    ...
    async getDataById(id, checkingEdit) {
        try {
            const responseData = await this.ajaxRequest(`${appUrl}/v1/kelas/get/${id}`, 'GET');
            console.log(checkingEdit())
            $('#modal-title').html('Edit Data')
            $('#id').val(responseData.data.id);
            $('#kelas').val(responseData.data.kelas);
            checkingEdit();
        } catch (error) {
            console.log(error);
        }
    ...
    async deleteData(id) {
        try {
            ...
            if (result.isConfirmed) {
                const responseData = await this.ajaxRequest(`${appUrl}/v1/kelas/delete/${id}`, 'DELETE');
                console.log(responseData);
                if (responseData.status === 'success') {
