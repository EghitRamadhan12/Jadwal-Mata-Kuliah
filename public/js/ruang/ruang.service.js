class ruangService {
    ...
    async getAllData() {
        ...
        try {
            const responseData = await this.ajaxRequest(`${appUrl}/v1/ruang/`, 'GET');
                ...
                responseData.data.forEach((item, index) => {
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            <button class="btn btn-outline-primary btn-sm edit-modal mr-1" data-toggle="modal" data-target="#ruangModal" data-id="${item.id}">
    ...
    async upsertData(e, checkingEdit) {
        ...
        try {
            ...
            if (isEditMode) {
                const id = $('#id').val();
                const responseData = await this.ajaxRequest(`${appUrl}/v1/ruang/update/${id}`, 'POST', formData);
                ...
                const responseData = await this.ajaxRequest(`${appUrl}/v1/ruang/create`, 'POST', formData);
                console.log(responseData);
                if (responseData.status === 'success') {
            ...
            if (error.responseJSON.message === "Nama ruang sudah"){
                existAlert()
    ...
    async getDataById(id, checkingEdit) {
        try {
            const responseData = await this.ajaxRequest(`${appUrl}/v1/ruang/get/${id}`, 'GET');
            console.log(checkingEdit())
            $('#modal-title').html('Edit Data')
            $('#id').val(responseData.data.id);
            $('#ruang').val(responseData.data.ruang);
    ...
    async deleteData(id) {
        try {
            ...
            if (result.isConfirmed) {
                const responseData = await this.ajaxRequest(`${appUrl}/v1/ruang/delete/${id}`, 'DELETE');
                console.log(responseData);
                if (responseData.status === 'success') {
